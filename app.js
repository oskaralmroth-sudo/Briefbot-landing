...
function render() {
  ...
  if (state.token) {
    ...
    switch (page) {
      ...
      case 'pricing': renderPricing(true); break;
      case 'reset': renderResetPassword(); break;
      default:
...
function renderPricing(loggedOut) {
  const sub = state.subscription || {};
  ...
  const target = loggedOut ? 'page-pricing-out' : 'page-pricing';
  const isLoggedOut = loggedOut || !state.token;

  if (isLoggedOut) {
    // Landing-style pricing for logged-out visitors
    document.getElementById('page-pricing-out').innerHTML = `
    <div class="landing-pricing">
      <div class="container">
        <h1>Välj din plan</h1>
        <p class="subtitle">Börja gratis — ingen bindningstid. Uppgradera när du växer.</p>

        <div class="plans-grid centered">
          <div class="pricing-card">
            ...
            <a href="#signup" class="btn btn-outline" onclick="navigate('signup')" style="width:100%;justify-content:center">Starta gratis</a>
          </div>
          ...
          <div class="pricing-card pricing-featured">
            ...
            <ul>
              ...
              <li>Företagsanpassade briefs</li>
            </ul>
            <a href="#signup" class="btn btn-primary" onclick="navigate('signup')" style="width:100%;justify-content:center">Starta gratis provperiod</a>
          </div>
          ...
          <div class="pricing-card">
            ...
            <a href="#signup" class="btn btn-outline" onclick="navigate('signup')" style="width:100%;justify-content:center">Kontakta oss</a>
          </div>
        </div>

        <div class="pricing-faq" style="margin-top:3rem;text-align:center;color:var(--text-muted)">
          <p>💳 Alla priser i SEK. Ingen bindningstid. Avsluta när du vill.</p>
          <p style="margin-top:0.5rem">❓ Frågor? <a href="#faq" onclick="navigate('faq')" style="color:var(--primary)">Kolla FAQ</a></p>
        </div>
      </div>
    </div>
    `;
